...
function agregarPersonal(event) {
   ...
   var telefono = document.getElementById('LetraEP').value;
   ...
   xhttp.send("nombre=" + encodeURIComponent(nombre) +
       ...
       "&email=" + encodeURIComponent(telefono) +    
...
/*--------------------------------------------------------------------------------------------------*/
/* Calculo de Datos de Ingrediente */

function agregarIngrediente(event) {
    event.preventDefault();

    var nombre = document.getElementById('LetraNP').value;
    var cantidad = document.getElementById('LetraD').value;

    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            alert(this.responseText); // Mostrar mensaje del servidor
    ...
    // Enviar los datos
    xhttp.send("LetraNP=" + encodeURIComponent(nombre) + "&LetraD=" + encodeURIComponent(cantidad));

   // Enviar los datos al servidor
   xhttp.send("nombre=" + encodeURIComponent(nombre) +
       "&direccion=" + encodeURIComponent(cantidad));

   // Resto del código para agregar la fila a la tabla
   var table = document.getElementById('tablaIngrediente');
   var rowCount = table.rows.length;
   var row = table.insertRow(rowCount);
